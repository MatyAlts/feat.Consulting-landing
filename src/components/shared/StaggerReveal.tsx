...
export function StaggerReveal({ children, staggerDelay = 50, className = '', triggerOnce = true }: StaggerRevealProps) {
    ...
    useEffect(() => {
        if (ref.current) {
            ...
            words.forEach((word, index) => {
                (word as HTMLElement).style.transitionDelay = `${index * staggerDelay}ms`;
            });
        }
